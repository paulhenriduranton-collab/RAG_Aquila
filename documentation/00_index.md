...
| Évaluation par composant | `agent.py` instrumenté (capture des états intermédiaires) + RAGAS (`gemma2:2b`) + LLM-juge custom (`gemma4:12b` pour le grading, `gemma2:2b` pour NDCG/MRR/dédup/RRF) | Isole les 8 briques du pipeline agentique, sans jamais relancer retrieval/génération pour re-scorer |
...
│   ├── evaluate_components.py # Évaluation par composant (8 briques), à partir d'agentic_results_debug.json

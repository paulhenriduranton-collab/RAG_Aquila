...
# Cache BM25 — construit une seule fois par session
...
def _merge(
    ...
    """Fusion score direct : sémantique (si ≥ seuil) + BM25 normalisé pondéré."""
    ...
    max_bm25 = max((bm25_raw_scores[i] for i in bm25_indices), default=1.0)
    for idx in bm25_indices:
        text, meta = bm25_chunks[idx]
        scores[text] = scores.get(text, 0) + (bm25_raw_scores[idx] / max_bm25) * BM25_WEIGHT
        if text not in doc_map:
            doc_map[text] = Document(page_content=text, metadata=meta)

    ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)

    # Diversité : au plus 1 chunk par page source pour éviter de saturer les slots
...
def ask_question(question: str) -> str:
    ...
    # ── 3. Fusion RRF ─────────────────────────────────────────────────────
    final_docs, rrf_ranking = _merge(raw_semantic, top_bm25, bm25_scores, bm25_chunks)
